var app = new function() {
  ...
  this.mostrarVuelos = function() {
    ...
    data += '<td><button onclick="app.Reservar(' + i + ')">Reservar</button></td>';
    data += '<td><button onclick="app.CancelarReserva(' + i + ')">Cancelar Reserva</button></td>'; // Agregar este botón
    ...
    document.getElementById('reserva-edit').onsubmit = function() {
      ...
      if (isNaN(d) || d == 0) {
        ...
        if (flag) {
          ...
          window.alert("Combi reservada correctamente");
  ...
  this.CancelarReserva = function (item) {
    var el = document.getElementById('documento');
    document.getElementById('documento').value = "";
    document.getElementById('datosvuelo').style.display = 'block';
    document.getElementById('vuelos').style.display = 'none';
    ...
    var self = this;
    document.getElementById('cancelar-edit').onsubmit = function() {
      var d = el.value * 1;
      if (isNaN(d) || d === 0) {
        window.alert("Ingrese un dato correcto");
      } else {
        var auxDoc = self.vuelos[item].reservas.indexOf(d);
        if (auxDoc !== -1) {
          self.vuelos[item].reservas.splice(auxDoc, 1); // Elimina la reserva
          window.alert("Reserva cancelada correctamente");
        } else {
          window.alert("Error: No se encontró una reserva con ese documento");
        }
